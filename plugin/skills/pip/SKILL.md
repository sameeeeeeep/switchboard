...
**never opening the agent app.** It is [[adhd-pm]]'s operator loop, made ambient and always-on.
...
- **GURU (model, on-demand):** every decision / question / handback is a notch card, exactly like
  the sibling `switchboard` skill — but now for the WHOLE session, not one reply. The user answers at the notch; nothing gets
  buried in chat.

## On `/pip` (turn it ON)

1. Call `switchboard_status`; if the app is down, open the installed Switchboard app and re-check.
2. Call `switchboard_pip({active:true})`, then `switchboard_notch` with an opening event and honest source.
3. **Adopt PIP behaviour for the rest of the session** (this is the mode — hold it until `/pip off`):
   ...
   - **Always route decisions through the notch** (the shared Switchboard workflow): ANY fork,
     ...
     `switchboard_result({surface:"guide",runId})`. NEVER end a turn with a decision sitting in chat. Chat is only the
...
1. **Never end a turn with just a chat message.** Before handing back, raise an **"over to you"** notch
   ...
   **When a handoff is relevant, include a `🔀 Handoff this thread` option** that runs [[handoff]] (commit+push + reconcile the
...
2. **Poll for the response** (`switchboard_result({surface:"guide",runId})`) — read BOTH `chosenOption` and the typed
...
4. **If the poll times out** (the user stepped away), the card + the durable log (`guide-history.jsonl`)
   hold the state — recover their answer next time with `switchboard_result` with the original runId. Don't declare done; you're waiting.
...
`switchboard_pip({active:false})`, or just tap the feed's
...
- **Every decision is a card, no exceptions.** In PIP mode there is no "just answer in chat" — the user
  isn't looking at chat. If a run gets clobbered, recover the answer with `switchboard_result` with the original runId (the durable log).
