...
if (!slug || !src) { console.error("usage: node deploy-wrapp.mjs <slug> <src-dir-or-html> [--go] [--owner X] [--domain sameep.ai]"); process.exit(1); }
...
if (!/^[a-z0-9][a-z0-9-]{0,40}$/.test(slug)) { console.error(`bad slug "${slug}" — lowercase letters, digits, hyphens only (it becomes ${slug}.${DOMAIN}).`); process.exit(1); }
...
console.log(`\n${DRY ? "DRY RUN — plan for" : "Deploying"} ${host}  (owner: ${owner}, repo: ${owner}/${slug})\n`);
...
run("git", ["-C", work, "commit", "-m", `deploy ${slug} → ${host}`]);
run("gh", ["repo", "create", `${owner}/${slug}`, "--public", "--source", work, "--remote", "origin", "--push"]);
console.log("3. enable GitHub Pages from main / root, set the custom domain:");
run("gh", ["api", "-X", "POST", `/repos/${owner}/${slug}/pages`, "-f", "source[branch]=main", "-f", "source[path]=/"], { allowFail: true });
run("gh", ["api", "-X", "PUT", `/repos/${owner}/${slug}/pages`, "-f", `cname=${host}`], { allowFail: true });

console.log(`\nDNS — add this ONE record at your ${DOMAIN} registrar (I don't touch DNS):`);
