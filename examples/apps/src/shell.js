// SHELL — describe a task, get the exact shell command(s) + a one-line explanation of each flag, all
// on the visitor's OWN Claude, and never anything destructive without a warning. The operator holds
...
// God's hands: expose Shell's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "shell",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Shell",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Shell — turns a task into the exact shell command(s) with each flag explained on your own Claude",
    models: ["sonnet"],
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// SHELL — describe a task → one stage produces the exact shell command(s) plus a one-line-per-flag
// explanation, streamed as markdown. Anything destructive gets a bold ⚠ warning ABOVE the command.
// Steering rewrites (one-liner, use find, macOS, add a dry-run…). Text-only.

const STEER_CHIPS = ["one-liner", "safer / dry-run first", "assume macOS", "explain more"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "Find every .log file under the current directory larger than 100MB and show them sorted by size.";
let running = false;

function autostart() {
  // No cold open — Shell needs a task to translate. Left in place for house-consistency.
  if (state.run) return;
}
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Describe the task first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "composing the command…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Give the exact shell command(s) for the task below.`,
        `THE TASK:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown with EXACTLY these bold headings, in order: **Command** (the exact command(s) in a fenced bash code block) and **Flags** (one line per flag/argument explaining what it does). SAFETY: if any command is destructive or irreversible (rm, dd, mkfs, force-push, DROP, truncate, chmod -R, etc.), put a bold line starting with ⚠ ABOVE the Command section warning the user and, where possible, suggest a non-destructive dry-run first. Prefer portable POSIX syntax unless the task names a platform.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Command copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample task (connect to solve your own)"));
  ...
  if (!r) {
    ...
    const btn = el("button", "primary", "Give me the command ⌘"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · command · every flag explained · warns before destructive"));
  ...
  bar.append(el("span", "kicker", "composing"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the command"));
...
// `shell_run` runs the SAME start() a type-and-go click runs — the command streams live in the DOM —
// then returns the command(s) and flag explanations for God to speak or file. Destructive commands
// carry an inline ⚠ warning; God should never run them without the user's say-so.
exposeToGod({
  name: "shell_run",
  description: "Give the exact shell command(s) for a task, with each flag explained — and a warning above anything destructive. Writes it live on the page and returns it.",
  inputSchema: { task: "string — describe the task you want a shell command for. Required." },
  execute: async ({ task } = {}) => {
    const input = String(task || "").trim();
    if (!input) throw new Error("no task — pass { task } describing what you want to do");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Shell isn't connected to Switchboard yet");
    ...
    throw new Error("Shell stayed busy — try again");
  },
});
