// MEETING NOTES — paste a transcript or rough notes, get clean STRUCTURED notes (TL;DR, decisions,
// action items with owners/dues, open questions) on the visitor's OWN Claude. The operator holds no
// key, pays for no inference, and never sees the user's data — Switchboard brokers everything.
// Granola, but private + one-click.
//
...
// God's hands: expose Meeting Notes' one action as a page-tool so the native God webview (or any
...
const APP = {
  id: "meetnotes",                              // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Meeting Notes",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Meeting Notes — reads a transcript or rough notes you paste and structures clean notes + action items on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = notes on the brand's own material
};
...
function connectSteps() {
  ...
  const s2 = el("div"); s2.innerHTML = "<b>2</b> · Paste a transcript — the notes structure themselves";
  const s3 = el("div"); s3.innerHTML = "<b>3</b> · TL;DR, decisions, action items, open questions — copy and go";
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// MEETING NOTES — paste a raw transcript or rough notes → one stage produces clean, structured
// meeting notes (TL;DR, decisions, action items with owners/dues, open questions), streamed as
// markdown. Steering re-runs it (shorter, add owners, be terser…). THE COLD OPEN: a lent brand
// context autostarts notes off the brand's own material. Text-only — no tools, no image gen.

const STEER_CHIPS = ["shorter", "more action items", "add owners", "just the decisions"];
...
function autostart() {
  // THE COLD OPEN — when a brand context is lent, structure notes off the brand's own material with
  // ZERO input. Fire only when useful, never over a saved run.
  if (state.run) return;
  if (brand) {
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the transcript or notes first.", true); return; }
  ...
  try {
    const text = await streamText({
      prompt: [
        ...
        r.fromContext ? `(This is the user's OWN brand material — structure notes on what it says and implies.)` : "",
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Use EXACTLY these markdown sections, in order, each as a bold heading: **TL;DR** (2-3 bullets), **Decisions** (bullets — what was actually decided; write 'None stated' if there are none), **Action items** (checkbox bullets in the form `- [ ] task — owner (due)`, filling in owner and due ONLY where the text implies them), **Open questions** (bullets — anything raised but unresolved). Be faithful to the source; don't invent decisions, owners, dues, or facts that aren't there.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Notes copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample transcript (connect for notes on your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to structure notes on its material"));
    const row = el("div", "bindrow");
    const input = el("textarea");
    input.rows = 4;
    input.placeholder = "paste a meeting transcript or rough notes…";
    ...
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · TL;DR · decisions · action items · open questions"));
  ...
  bar.append(el("span", "kicker", "notes from"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the notes"));
...
// `meetnotes_run` runs the SAME start() a paste-and-go click runs — the notes stream live in the DOM
// — then returns the structured notes for God to speak or file.
exposeToGod({
  name: "meetnotes_run",
  description: "Turn a meeting transcript or rough notes into clean structured notes (TL;DR, decisions, action items with owners/dues, open questions). Writes it live on the page and returns it.",
  inputSchema: { transcript: "string — the meeting transcript or rough notes to structure. Required." },
  execute: async ({ transcript } = {}) => {
    const input = String(transcript || "").trim();
    if (!input) throw new Error("nothing to structure — pass { transcript } with the meeting transcript or notes");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Meeting Notes isn't connected to Switchboard yet");
    ...
    throw new Error("Meeting Notes stayed busy — try again");
  },
});
