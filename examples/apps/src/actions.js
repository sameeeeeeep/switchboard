// ACTION ITEMS — paste meeting notes or a transcript and get a clean action-item checklist, with
// owners and due dates where implied, on the visitor's OWN Claude. The operator holds no key, pays
...
// God's hands: expose Action Items' one action as a page-tool so the native God webview (or any
...
const APP = {
  id: "actions",                                // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Action Items",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Action Items — reads notes or a transcript you paste and pulls a clean checklist on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = pull actions in the brand's own frame
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// ACTION ITEMS — paste meeting notes or a transcript → one stage pulls a clean action-item checklist,
// with owners and due dates where they're implied, streamed as a markdown checklist. Steering
// re-extracts (group by owner, only mine, add priorities…). THE COLD OPEN: a lent brand context
// autostarts a checklist from the brand's own plans. Text-only — no tools, no image gen.

const STEER_CHIPS = ["group by owner", "add priorities", "only decisions", "shorter"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "Sarah: I'll get the pricing deck to legal by Friday. Tom said he'd loop in the design team next week. We agreed to push the launch to the 15th. Someone needs to email the beta list — Maya can you take that? Also still open: who owns the analytics dashboard.";
let running = false;

function autostart() {
  // THE COLD OPEN — when a brand context is lent, pull actions from the brand's own plans with ZERO
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the notes or transcript first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "pulling the action items…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Turn the notes/transcript below into a clean, deduplicated action-item checklist.`,
        `THE NOTES:\n"""${r.input.slice(0, 8000)}"""`,
        r.fromContext ? `(This is the user's OWN brand plan — extract the concrete next steps it implies.)` : "",
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Output a markdown checklist: one `- [ ] ` line per action, each a clear imperative. Where an owner is named or clearly implied, append ` — **@owner**`; where a due date/deadline is stated or implied, append ` (due …)`. Only include owners/dates the text actually supports — never invent them. If some items have no owner, list them under a short `Unassigned` note. Keep it faithful and tight; no preamble.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1200,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Checklist copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample notes (connect to run your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to pull its next steps"));
    const row = el("div", "bindrow");
    const input = el("textarea");
    input.rows = 3;
    input.placeholder = "paste meeting notes or a transcript…";
    ...
    const btn = el("button", "primary", "Pull actions ✅"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · owners and due dates where implied"));
  ...
  bar.append(el("span", "kicker", "extracting"), el("span", "run-input", r.input), el("span", "grow"));
...
// `actions_run` runs the SAME start() a paste-and-go click runs — the checklist streams live in the
// DOM — then returns the markdown checklist for God to speak or file.
exposeToGod({
  name: "actions_run",
  description: "Turn meeting notes or a transcript into a clean action-item checklist (owners and due dates where implied). Writes it live on the page and returns it.",
  inputSchema: { notes: "string — the notes or transcript to extract action items from. Required." },
  execute: async ({ notes } = {}) => {
    const input = String(notes || "").trim();
    if (!input) throw new Error("nothing to extract — pass { notes } with the notes or transcript");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Action Items isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.out || r.error)) {
        ...
        return { checklist: r.out };
      }
    }
    throw new Error("Action Items stayed busy — try again");
  },
});
