// EXPLAIN THIS — paste anything confusing, get it in plain English for a smart non-expert, on the
...
// God's hands: expose Explain This's one action as a page-tool so the native God webview (or any
// WebMCP host) can DRIVE it — reusing the same start() a click runs, so the user watches it happen.
import { exposeToGod } from "./kit/webmcp.js";

// ==== CONFIG — every new wrapp edits this block =============================================
const APP = {
  id: "explainthis",                            // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Explain This",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Explain This — reads text you paste and explains it in plain English on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = explain the brand's own positioning
  placeholder: "paste anything confusing — jargon, a clause, a paragraph, an error…",
  extraField: null,
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// EXPLAIN THIS — paste anything confusing → stream a plain-English explanation for a smart
// non-expert, as markdown. Steering re-runs the explanation. THE COLD OPEN: a lent brand context
// autostarts an explanation of the brand's own positioning with zero input. Text-only.

const STEER_CHIPS = ["simpler", "shorter", "use an analogy", "more detail"];
const SAMPLE = "The counterparty agrees to indemnify and hold harmless the disclosing party against any and all claims arising from the recipient's breach of the confidentiality provisions, subject to the limitation of liability set forth in Section 9.";
let running = false;
...
async function start(input, extra, fromContext) {
  ...
  if (!input) { toast("Paste something to explain first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.output = ""; r.status = "making it plain…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Explain the text below in plain English to a smart non-expert.`,
        `TEXT:\n"""${r.input.slice(0, 8000)}"""`,
        r.fromContext ? `(This is the user's OWN brand — explain plainly what it does and who it's for.)` : "",
        r.steer ? `Adjust per this instruction: "${r.steer}".` : "",
        "Be clear and concrete: define any jargon, use everyday language, stay accurate, and don't condescend. No preamble. Markdown.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1200,
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample (connect to explain your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to explain its positioning"));
    ...
    const btn = el("button", "primary", "Explain it"); btn.onclick = go;
    startBox.append(btn);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter to run · plain English, no condescension"));
  ...
  bar.append(el("span", "kicker", "explaining"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.output) {
    col.append(el("div", "kicker sect", "in plain english"));
...
exposeToGod({
  name: "explainthis_run",
  description: "Explain a bit of text in plain English for a smart non-expert. Writes it live on the page and returns it.",
  inputSchema: { text: "string — the text to explain. Required." },
  execute: async ({ text } = {}) => {
    const input = String(text || "").trim();
    if (!input) throw new Error("nothing to explain — pass { text } with the text to explain");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Explain This isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.output || r.error)) {
        ...
        return { explanation: r.output };
      }
    }
    throw new Error("Explain This stayed busy — try again");
  },
});
