// OUTLINE — turns a topic into a clean structure for a post, talk, or doc on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "outline",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Outline",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Outline — turns a topic into a clean structure for a post, talk, or doc on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["more detailed", "tighter", "add a hook + CTA", "for a talk"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "A conference talk on why open-source AI tools feel built for developers, not consumers — and how to close the gap.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Tell it what you're writing or presenting first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "structuring it…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Produce a clean outline for the piece described below.`,
        `THE PIECE:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown: a suggested title line, then a hierarchical outline — top-level sections, each with 2–4 bullet sub-points. End with a **Hook** line (a one-sentence opening) and a **CTA** line (a one-sentence close). Match the shape to the medium (post / talk / doc) if the topic names one.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Outline copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample topic (connect to outline your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "what are you writing or presenting?…";
    ...
    const btn = el("button", "primary", "Outline it 🗂️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · title · sections · hook + CTA"));
  ...
  bar.append(el("span", "kicker", "outlining"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the outline"));
...
exposeToGod({
  name: "outline_run",
  description: "Produce a clean hierarchical outline for a post, talk, or doc, with a suggested hook and CTA. Writes it live on the page and returns it.",
  inputSchema: { topic: "string — what you're writing or presenting. Required." },
  execute: async ({ topic } = {}) => {
    const input = String(topic || "").trim();
    if (!input) throw new Error("nothing to outline — pass { topic } describing the piece");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Outline isn't connected to Switchboard yet");
    ...
    throw new Error("Outline stayed busy — try again");
  },
});
