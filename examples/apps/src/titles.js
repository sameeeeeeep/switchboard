// TITLES — turns a topic into headline and title options you can A/B on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "titles",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Titles",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Titles — turns a topic into headline and title options you can A/B on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["more clickable", "more serious", "add numbers", "shorter"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "A blog post on how we cut our AI inference bill to zero by having users bring their own Claude.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Tell it what the piece is about first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing titles…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write headline / title options for the topic below.`,
        `THE TOPIC:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown as a numbered list of 8 titles spanning styles — how-to, listicle, contrarian, benefit-led, question, bold claim, curiosity, plainspoken. Mark the single strongest with **★ Recommended** and one line on why. Keep every title honest — no clickbait the content can't pay off.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Titles copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample topic (connect to title your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "what's the piece about?…";
    ...
    const btn = el("button", "primary", "Write titles 🏷️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · 8 titles · one recommended"));
  ...
  bar.append(el("span", "kicker", "writing titles"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the titles"));
...
exposeToGod({
  name: "titles_run",
  description: "Write 8 headline/title options for a topic across styles, one marked recommended. Writes them live on the page and returns them.",
  inputSchema: { topic: "string — what the piece is about. Required." },
  execute: async ({ topic } = {}) => {
    const input = String(topic || "").trim();
    if (!input) throw new Error("nothing to title — pass { topic } describing the piece");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Titles isn't connected to Switchboard yet");
    ...
    throw new Error("Titles stayed busy — try again");
  },
});
