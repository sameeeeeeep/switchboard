// OBJECTION — turns a sales or investor objection into a calm, honest response on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "objection",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Objection",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Objection — turns a sales or investor objection into a calm, honest response on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["more concise", "warmer", "more data-driven", "acknowledge more"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "\"Why wouldn't Anthropic just build this themselves and kill you?\"";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the objection first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "framing the response…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Help handle the objection below honestly — never with spin.`,
        `THE OBJECTION:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown with EXACTLY these bold headings, in order: **What they're really asking** (the underlying concern in one line), **The response** (a calm, specific answer, 2–4 sentences), **One-liner** (a single crisp sentence to say out loud). Be truthful; if the objection has real merit, acknowledge it and address it rather than deflecting.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Response copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample objection (connect to handle your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "paste the objection you're facing…";
    ...
    const btn = el("button", "primary", "Handle it 🛡️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · reframe · answer · one-liner"));
  ...
  bar.append(el("span", "kicker", "handling objection"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the response"));
...
exposeToGod({
  name: "objection_run",
  description: "Reframe a sales or investor objection and draft a calm, honest response plus a one-liner. Writes it live on the page and returns it.",
  inputSchema: { objection: "string — the objection you're facing. Required." },
  execute: async ({ objection } = {}) => {
    const input = String(objection || "").trim();
    if (!input) throw new Error("nothing to handle — pass { objection } with the objection");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Objection isn't connected to Switchboard yet");
    ...
    throw new Error("Objection stayed busy — try again");
  },
});
