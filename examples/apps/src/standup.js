// STANDUP — turns your notes or commits into a crisp standup update on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "standup",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Standup",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Standup — turns your notes or commits into a crisp standup update on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["tighter", "more detail", "add blockers", "for Slack"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "yesterday: shipped the creator skill batch, fixed the harness driver. today: founder batch + morning report. stuck on nothing.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste your notes or commits first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing the update…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Turn the raw notes below into a crisp daily standup update.`,
        `THE NOTES:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown with EXACTLY these bold headings, in order: **Yesterday** (bullets of what got done), **Today** (bullets of the plan), **Blockers** (bullets, or 'None'). Keep each bullet one line, concrete, no fluff. Never invent work that isn't in the notes.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Standup copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample notes (connect to write your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "paste your notes, commits, or bullet points…";
    ...
    const btn = el("button", "primary", "Write standup 🧵"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · yesterday · today · blockers"));
  ...
  bar.append(el("span", "kicker", "writing standup"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the update"));
...
exposeToGod({
  name: "standup_run",
  description: "Turn raw notes or commits into a crisp Yesterday / Today / Blockers standup. Writes it live on the page and returns it.",
  inputSchema: { notes: "string — your raw notes, commits, or bullets. Required." },
  execute: async ({ notes } = {}) => {
    const input = String(notes || "").trim();
    if (!input) throw new Error("nothing to summarize — pass { notes } with your raw notes");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Standup isn't connected to Switchboard yet");
    ...
    throw new Error("Standup stayed busy — try again");
  },
});
