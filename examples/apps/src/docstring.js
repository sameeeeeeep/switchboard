// DOCSTRING — paste a function, get a clear docstring / JSDoc (params, returns, one example) on the
...
// God's hands: expose Docstring's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "docstring",                              // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Docstring",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Docstring — writes a clear docstring/JSDoc (params, returns, example) for a function on your own Claude",
    models: ["sonnet"],
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// DOCSTRING — paste a function → one stage writes a clear docstring / JSDoc in the language's own
// convention: every param, the return value, and one usage example, streamed as a fenced code block.
// Steering rewrites (terser, add types, Google style, NumPy style…). Text-only.

const STEER_CHIPS = ["terser", "add types", "Google style", "include edge cases"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "function debounce(fn, wait) {\n  let t;\n  return (...args) => {\n    clearTimeout(t);\n    t = setTimeout(() => fn(...args), wait);\n  };\n}";
let running = false;

function autostart() {
  // No cold open — Docstring needs a function to document. Left in place for house-consistency.
  if (state.run) return;
}
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the function first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "documenting the function…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write a clear docstring / JSDoc for the function below.`,
        `THE CODE:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply with ONLY the docstring/JSDoc inside a single fenced code block, in the documentation convention that matches the code's language (JSDoc for JS/TS, triple-quoted for Python, etc.). Document a one-line summary, every parameter (name, type, meaning), the return value, and include one short @example. Be accurate to the code — never document parameters that don't exist or return values the function doesn't produce.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Docstring copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample function (connect to document your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "paste the function…";
    ...
    const btn = el("button", "primary", "Document it 📝"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · params · returns · example"));
  ...
  bar.append(el("span", "kicker", "documenting"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the docstring"));
...
// `docstring_run` runs the SAME start() a paste-and-go click runs — the docstring streams live in the
// DOM — then returns it for God to speak or file.
exposeToGod({
  name: "docstring_run",
  description: "Write a clear docstring/JSDoc for a function — params, returns, one example — in the language's own convention. Writes it live on the page and returns it.",
  inputSchema: { code: "string — the function source to document. Required." },
  execute: async ({ code } = {}) => {
    const input = String(code || "").trim();
    if (!input) throw new Error("nothing to document — pass { code } with the function source");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Docstring isn't connected to Switchboard yet");
    ...
    throw new Error("Docstring stayed busy — try again");
  },
});
