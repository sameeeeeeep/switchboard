// TRANSLATE — paste text, get it in the target language with tone preserved, on the visitor's OWN
// Claude. The operator holds no key, pays for no inference, and never sees the user's data —
...
// God's hands: expose Translate's one action as a page-tool so the native God webview (or any WebMCP
// host) can DRIVE it — reusing the same start() a click runs, so the user watches it happen.
import { exposeToGod } from "./kit/webmcp.js";

// ==== CONFIG — every new wrapp edits this block =============================================
const APP = {
  id: "translate",                              // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Translate",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Translate — translates text you paste into a target language on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = translate the brand's own positioning
  placeholder: "paste the text you want translated…",
  extraField: { placeholder: "target language — e.g. 'Spanish', 'Japanese', 'French' (default: Spanish)" },
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// TRANSLATE — paste text (+ a target language) → stream the translation, tone and register
// preserved, as markdown. Steering re-runs the translation. THE COLD OPEN: a lent brand context
// autostarts a translation of the brand's own positioning with zero input. Text-only.

const STEER_CHIPS = ["more formal", "more casual", "shorter", "more natural"];
const SAMPLE = "Thanks so much for hopping on the call today — I really enjoyed learning about your team's roadmap. I'll send over the proposal by Friday, and let's find time next week to talk through the details.";
let running = false;
...
async function run(steer) {
  ...
  const lang = r.extra || "Spanish";
  running = true; r.error = null; r.output = ""; r.status = "translating into " + lang + "…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}, a fluent, idiomatic translator.`,
        `Translate the text below into ${lang}, preserving its tone, register, and meaning.`,
        `TEXT:\n"""${r.input.slice(0, 8000)}"""`,
        r.fromContext ? `(This is the user's OWN brand — translate its positioning naturally for that market.)` : "",
        r.steer ? `Adjust per this instruction: "${r.steer}".` : "",
        "Output ONLY the translation — no notes, no source text, no transliteration unless idiomatic. Markdown allowed.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1400,
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample (connect to translate your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to translate its positioning"));
    ...
    const btn = el("button", "primary", "Translate it"); btn.onclick = go;
    startBox.append(btn);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter to run · tone preserved · defaults to Spanish"));
  ...
  bar.append(el("span", "kicker", "translating"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.output) {
    col.append(el("div", "kicker sect", "the translation" + (r.extra ? " · " + r.extra : "")));
...
exposeToGod({
  name: "translate_run",
  description: "Translate a bit of text into a target language, preserving tone. Writes it live on the page and returns it.",
  inputSchema: { text: "string — the text to translate. Required.", language: "string — target language (default: Spanish)." },
  execute: async ({ text, language } = {}) => {
    const input = String(text || "").trim();
    if (!input) throw new Error("nothing to translate — pass { text } with the text to translate");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Translate isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      await waitFor(() => !running, 180000);
      await start(input, language);
      await waitFor(() => !running, 180000);
      const r = state.run || {};
      if (r.input === input && (r.output || r.error)) {
        if (r.error) throw new Error(r.error);
        return { translation: r.output, language: r.extra || "Spanish" };
      }
    }
    throw new Error("Translate stayed busy — try again");
  },
});
