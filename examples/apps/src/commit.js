// COMMIT — paste a diff or describe a change, get a clean Conventional Commits message on the
...
// God's hands: expose Commit's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "commit",                                 // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Commit",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Commit — turns a diff or change summary into a Conventional Commits message on your own Claude",
    models: ["sonnet"],
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// COMMIT — paste a diff or a change summary → one stage writes a Conventional Commits message: a
// typed subject line plus a body of concise bullets, streamed as markdown. Steering rewrites (add
// scope, breaking change, terser body…). Text-only.

const STEER_CHIPS = ["add a scope", "mark breaking change", "terser body", "reference an issue"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "Added a retry with exponential backoff to the upload client, fixed a null check in the progress bar, and bumped the axios dependency.";
let running = false;

function autostart() {
  // No cold open — Commit needs a diff or change summary. Left in place for house-consistency.
  if (state.run) return;
}
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the diff or change summary first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing the message…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write a Conventional Commits message for the diff or change summary below.`,
        `THE CHANGE:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply with ONLY the commit message inside a single fenced code block: a Conventional Commits subject line (type(scope): subject — imperative mood, ≤72 chars) then a blank line, then a body of concise bullet points covering what changed and why. Pick the correct type (feat, fix, docs, refactor, chore, test, perf, build, ci). Add a scope only if the change clearly has one. Reason only from what's shown; don't invent changes.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Commit copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample change (connect to write your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "paste a diff or describe the change…";
    ...
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · conventional commits · subject + body"));
  ...
  bar.append(el("span", "kicker", "writing commit"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the commit"));
...
// `commit_run` runs the SAME start() a paste-and-go click runs — the message streams live in the DOM —
// then returns the Conventional Commits message for God to speak or file.
exposeToGod({
  name: "commit_run",
  description: "Write a Conventional Commits message (typed subject + body bullets) from a diff or change summary. Writes it live on the page and returns it.",
  inputSchema: { diff: "string — the diff or change summary to describe. Required." },
  execute: async ({ diff } = {}) => {
    const input = String(diff || "").trim();
    if (!input) throw new Error("nothing to commit — pass { diff } with the diff or change summary");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Commit isn't connected to Switchboard yet");
    ...
    throw new Error("Commit stayed busy — try again");
  },
});
