// POLISH — paste your writing, get it proofread and clarified while keeping your voice, on the
...
// God's hands: expose Polish's one action as a page-tool so the native God webview (or any WebMCP
// host) can DRIVE it — reusing the same start() a click runs, so the user watches it happen.
import { exposeToGod } from "./kit/webmcp.js";

// ==== CONFIG — every new wrapp edits this block =============================================
const APP = {
  id: "polish",                                 // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Polish",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Polish — proofreads and clarifies text you paste, keeping your voice, on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = polish the brand's own positioning
  placeholder: "paste your writing — an email, a post, a paragraph, a doc…",
  extraField: null,
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// POLISH — paste your writing → stream a proofread, clarified version that keeps your voice, as
// markdown. Steering re-runs the polish. THE COLD OPEN: a lent brand context autostarts a polish of
// the brand's own positioning copy with zero input. Text-only — no tools, no image gen.

const STEER_CHIPS = ["lighter touch", "more formal", "tighten it", "keep it casual"];
const SAMPLE = "Their product have alot of features but honestly it was'nt intuitive for me and my team, we spend to much time figuring out where things are and the onboarding could of been better. Overall its ok but their is room to improve.";
let running = false;
...
async function start(input, extra, fromContext) {
  ...
  if (!input) { toast("Paste some writing first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.output = ""; r.status = "polishing…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Proofread the text below and fix grammar, spelling, punctuation, and clarity — while keeping the author's own voice, tone, and intent intact.`,
        `TEXT:\n"""${r.input.slice(0, 8000)}"""`,
        r.fromContext ? `(This is the user's OWN brand copy — polish it, keep every claim and its voice.)` : "",
        r.steer ? `Adjust per this instruction: "${r.steer}".` : "",
        "Output ONLY the corrected text — no list of changes, no commentary. Preserve the original formatting and structure. Markdown allowed.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1400,
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample (connect to polish your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to polish its copy"));
    ...
    const btn = el("button", "primary", "Polish it"); btn.onclick = go;
    startBox.append(btn);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter to run · fixes grammar & clarity, keeps your voice"));
  ...
  bar.append(el("span", "kicker", "polishing"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.output) {
    col.append(el("div", "kicker sect", "polished"));
...
exposeToGod({
  name: "polish_run",
  description: "Proofread and clarify a bit of text while keeping the author's voice. Writes it live on the page and returns it.",
  inputSchema: { text: "string — the text to polish. Required." },
  execute: async ({ text } = {}) => {
    const input = String(text || "").trim();
    if (!input) throw new Error("nothing to polish — pass { text } with the text to polish");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Polish isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.output || r.error)) {
        ...
        return { polished: r.output };
      }
    }
    throw new Error("Polish stayed busy — try again");
  },
});
