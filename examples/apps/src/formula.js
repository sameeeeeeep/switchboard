// FORMULA — describe a spreadsheet need in plain English, get the exact formula (Google Sheets +
// Excel if they differ) plus one line explaining it, on the visitor's OWN Claude. The operator holds
...
// God's hands: expose Formula's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "formula",                                // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Formula",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Formula — turns a plain-English spreadsheet need into the exact formula on your own Claude",
    models: ["sonnet"],
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// FORMULA — describe a spreadsheet need in plain English → one stage writes the exact formula
// (Google Sheets, plus the Excel version if it differs) and one line explaining it, streamed as
// markdown. Steering re-writes it (Excel only, array formula, explain more…). Text-only — no tools.

const STEER_CHIPS = ["Excel only", "Google Sheets only", "array formula", "explain more"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "Sum the amounts in column B, but only the rows where column A says 'Paid'.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Describe the spreadsheet need first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing the formula…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write the exact spreadsheet formula for the need below.`,
        `THE NEED:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Give the exact formula for Google Sheets, and the Excel version too if it differs (say so when they're identical). Then one short line explaining what it does. Use markdown: a **Formula** section with the formula(s), then a **What it does** line. Assume A1-style references and state any assumption you make about which cells/ranges to use. Be precise — never invent functions that don't exist.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 700,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Formula copied ✓"); }
...
function render() {
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "describe what you want the formula to do…";
    ...
    const btn = el("button", "primary", "Write formula 🧮"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · Google Sheets + Excel · one line explaining it"));
  ...
  bar.append(el("span", "kicker", "formula for"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the formula"));
...
// `formula_run` runs the SAME start() a describe-and-go click runs — the formula streams live in the
// DOM — then returns it for God to speak or paste.
exposeToGod({
  name: "formula_run",
  description: "Write the exact spreadsheet formula for a plain-English need (Google Sheets + Excel), with one line explaining it. Writes it live on the page and returns it.",
  inputSchema: { need: "string — the spreadsheet need in plain English. Required." },
  execute: async ({ need } = {}) => {
    const input = String(need || "").trim();
    if (!input) throw new Error("nothing to solve — pass { need } with the spreadsheet need");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Formula isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.out || r.error)) {
        ...
        return { formula: r.out };
      }
    }
    throw new Error("Formula stayed busy — try again");
  },
});
