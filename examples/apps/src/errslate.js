// ERRSLATE — paste an error message or stack trace, get a plain-English decode: what it means, the
// likely cause, and the fix, all on the visitor's OWN Claude. The operator holds no key, pays for no
...
// God's hands: expose Errslate's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "errslate",                               // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Errslate",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Errslate — decodes an error or stack trace into meaning, likely cause, and the fix on your own Claude",
    models: ["sonnet"],
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// ERRSLATE — paste an error message or stack trace → one stage decodes it: what it means in plain
// language, the likely cause, and a concrete fix (with a snippet where useful), streamed as markdown.
// Steering re-decodes (shorter, more detail, assume TypeScript…). Text-only.

const STEER_CHIPS = ["more detail", "just the fix", "assume TypeScript", "explain like I'm new"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "TypeError: Cannot read properties of undefined (reading 'map')\n    at Dashboard (app/dashboard.js:42:19)\n    at renderWithHooks (react-dom.development.js:14985:18)";
let running = false;

function autostart() {
  // No cold open — Errslate needs an error to decode. Left in place for house-consistency.
  if (state.run) return;
}
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the error or stack trace first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "decoding the trace…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Decode the error message or stack trace below for a developer.`,
        `THE ERROR:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown with EXACTLY these bold headings, in order: **What it means** (1–2 sentences in plain language), **Likely cause** (bullets, most-likely first), **The fix** (concrete steps, with a short snippet in a fenced code block where it helps). Reason only from what the trace evidences; never invent framework internals or file details that aren't shown.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Decode copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample error (connect to decode your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "paste the error message or stack trace…";
    ...
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · meaning · cause · fix"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the decode"));
...
// `errslate_run` runs the SAME start() a paste-and-go click runs — the decode streams live in the DOM —
// then returns the meaning, cause, and fix for God to speak or file.
exposeToGod({
  name: "errslate_run",
  description: "Decode an error message or stack trace: what it means, the likely cause, and the fix. Writes it live on the page and returns it.",
  inputSchema: { error: "string — the error message or stack trace to decode. Required." },
  execute: async ({ error } = {}) => {
    const input = String(error || "").trim();
    if (!input) throw new Error("nothing to decode — pass { error } with the error or stack trace");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Errslate isn't connected to Switchboard yet");
    ...
    throw new Error("Errslate stayed busy — try again");
  },
});
