// CUT — paste a video's TRANSCRIPT (or rough notes) → the visitor's OWN Claude returns a CUT PLAN:
// punchy on-screen captions + a highlight cut list (keep in→out ranges, each with a one-line "why")
// + what to trim. The operator holds no key, pays for no inference, and never sees the user's data —
// Switchboard brokers everything. Skill-first + text-first: reliable today, no upload, zero extra cost.
//
// This file is TEMPLATE PLUMBING + the app. Everything between here and the "APP LOGIC" line is
// proven idiom (distilled from recap.js / roast.js) — keep it byte-identical. Edit the CONFIG block
// and everything below APP LOGIC.
//
// STAGE 2 (the real editor) = wire examples/cutstudio/index.html — the no-AI cut room: upload real
// footage, scrub a timeline, select and cut like an editor, export the HTML or a scroll-through
// .webm — plus kit/capture.js and an stt pass so a dropped video becomes a transcript automatically.
// v1 stays honest: transcript in → cut plan out, so the skill is dependable before the timeline lands.
//
...
// God's hands: expose Cut's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "cut",                                    // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Cut",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Cut — reads a video transcript you paste and drafts captions + a highlight cut list on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = plan cuts around the brand's own material
};
...
function connectSteps() {
  ...
  const s2 = el("div"); s2.innerHTML = "<b>2</b> · Paste a transcript — the cut plan writes itself";
  const s3 = el("div"); s3.innerHTML = "<b>3</b> · Keep the captions and cuts you like, steer the rest";
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// CUT — paste a video's TRANSCRIPT (or rough notes) → one stage produces a CUT PLAN as markdown:
// **Captions** (short punchy on-screen lines, one per beat), **Cut list** (keep MM:SS–MM:SS — the
// one-line reason it earns the cut), **Trim** (what to drop). Streamed live. Steering re-plans
// (punchier captions, tighter cuts, hook-first…). THE COLD OPEN: a lent brand context autostarts a
// plan for a short clip about the brand's own material. Text-only — no tools, no upload, no cost.

const STEER_CHIPS = ["punchier captions", "tighter cuts", "hook first", "under 60s"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "00:00 So today I want to talk about why most onboarding flows fail. 00:14 The number one mistake is asking for too much up front. 00:31 We cut our signup fields from nine to three and activation jumped forty percent. 00:52 Here's the exact three fields we kept and why. 01:20 And the one field everyone thinks they need but actually kills conversion.";
let running = false;

function autostart() {
  // THE COLD OPEN — when a brand context is lent, draft a cut plan for a short clip about the
  // brand's own material with ZERO input. Fire only when useful, never over a saved run.
  if (state.run) return;
  if (brand) {
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the transcript to cut first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "reading the transcript, planning the cut…"; render();
  try {
    const text = await streamText({
      prompt: [
        ...
        r.fromContext ? `(This is the user's OWN brand material — plan a short clip that sells what it says, faithfully.)` : "",
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Cut plan copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample transcript (connect to cut your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to plan a clip from its material"));
    const row = el("div", "bindrow");
    const input = el("textarea");
    input.rows = 3;
    input.placeholder = "paste a video transcript (with timestamps if you have them) or rough notes…";
    ...
    const btn = el("button", "primary", "Cut it ✂️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · captions · cut list · trim"));
  ...
  bar.append(el("span", "kicker", "cutting"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the cut plan"));
...
// `cut_run` runs the SAME start() a paste-and-go click runs — the cut plan streams live in the DOM —
// then returns the plan for God to speak or file.
exposeToGod({
  name: "cut_run",
  description: "Turn a video transcript (or rough notes) into a cut plan: on-screen captions + a highlight cut list (keep MM:SS–MM:SS with a reason) + what to trim. Writes it live on the page and returns it.",
  inputSchema: { transcript: "string — the video transcript or rough notes to plan a cut from. Required." },
  execute: async ({ transcript } = {}) => {
    const input = String(transcript || "").trim();
    if (!input) throw new Error("nothing to cut — pass { transcript } with the transcript or notes");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Cut isn't connected to Switchboard yet");
    ...
    throw new Error("Cut stayed busy — try again");
  },
});
