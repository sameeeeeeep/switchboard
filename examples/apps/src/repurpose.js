// REPURPOSE — turns one piece of content into tuned versions for other channels on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "repurpose",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Repurpose",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Repurpose — turns one piece of content into tuned versions for other channels on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["more casual", "punchier", "add hashtags", "for X/Twitter"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "We cut our AI inference bill to zero by having every user bring their own Claude. The wrapper ships the UX; the user brings the compute. No resold keys, no markup.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Paste the content to repurpose first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "repurposing…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Take the content below and rewrite it for other channels, adapting tone and length per channel.`,
        `THE CONTENT:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown with EXACTLY these bold headings, in order: **X/Twitter thread** (a numbered list of tweets), **LinkedIn post** (a few short paragraphs), **Instagram caption** (with a few hashtags), **Short-form video script** (spoken beats, one per line). Preserve the core message faithfully; never invent facts not in the source.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Repurposed copy ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample content (connect to repurpose your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "paste the content to repurpose…";
    ...
    const btn = el("button", "primary", "Repurpose ♻️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · thread · post · caption · script"));
  ...
  bar.append(el("span", "kicker", "repurposing"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the versions"));
...
exposeToGod({
  name: "repurpose_run",
  description: "Rewrite one piece of content into an X thread, a LinkedIn post, an Instagram caption, and a short-form video script. Writes them live on the page and returns them.",
  inputSchema: { content: "string — the content to repurpose. Required." },
  execute: async ({ content } = {}) => {
    const input = String(content || "").trim();
    if (!input) throw new Error("nothing to repurpose — pass { content } with the source");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Repurpose isn't connected to Switchboard yet");
    ...
    throw new Error("Repurpose stayed busy — try again");
  },
});
