// GIST — paste anything, get a tight TL;DR (2 sentences + up to 4 bullets), on the visitor's OWN
...
// God's hands: expose Gist's one action as a page-tool so the native God webview (or any WebMCP
// host) can DRIVE it — reusing the same start() a click runs, so the user watches it happen.
import { exposeToGod } from "./kit/webmcp.js";

// ==== CONFIG — every new wrapp edits this block =============================================
const APP = {
  id: "gist",                                   // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Gist",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Gist — reads text you paste and summarizes it into a tight TL;DR on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = summarize the brand's own positioning
  placeholder: "paste anything — an article, an email thread, notes, a doc…",
  extraField: null,
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// GIST — paste anything → stream a tight TL;DR (2 sentences + up to 4 bullets) as markdown. Steering
// re-runs the summary. THE COLD OPEN: a lent brand context autostarts the summary of the brand's own
// positioning with zero input. Text-only — no tools, no image gen.

const STEER_CHIPS = ["shorter", "just bullets", "one sentence", "more detail"];
const SAMPLE = "We're building the operating system for small teams — an all-in-one workspace that unifies chat, docs, tasks, and goals so companies stop paying for six tools that don't talk to each other. Early customers cut tool spend 40% and onboard new hires in a day.";
let running = false;
...
async function run(steer) {
  ...
  running = true; r.error = null; r.output = ""; r.status = "reading it…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Summarize the text below into a tight TL;DR.`,
        `TEXT:\n"""${r.input.slice(0, 8000)}"""`,
        r.fromContext ? `(This is the user's OWN brand — summarize its positioning and offer.)` : "",
        r.steer ? `Adjust per this instruction: "${r.steer}".` : "",
        "Output exactly: a 2-sentence TL;DR, then up to 4 short bullets of the key points. No preamble, no heading. Markdown.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 900,
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample (connect to gist your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to summarize its positioning"));
    ...
    const btn = el("button", "primary", "Get the gist"); btn.onclick = go;
    startBox.append(btn);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter to run · tight TL;DR + key bullets"));
  ...
  bar.append(el("span", "kicker", "gist"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.output) {
    col.append(el("div", "kicker sect", "the gist"));
...
exposeToGod({
  name: "gist_run",
  description: "Summarize a bit of text into a tight TL;DR (2 sentences + up to 4 bullets). Writes it live on the page and returns it.",
  inputSchema: { text: "string — the text to summarize. Required." },
  execute: async ({ text } = {}) => {
    const input = String(text || "").trim();
    if (!input) throw new Error("nothing to summarize — pass { text } with the text to gist");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Gist isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.output || r.error)) {
        ...
        return { gist: r.output };
      }
    }
    throw new Error("Gist stayed busy — try again");
  },
});
