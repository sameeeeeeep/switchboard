// COLD EMAIL — turns a target and an ask into a short, non-cringe cold email on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "coldemail",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Cold Email",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Cold Email — turns a target and an ask into a short, non-cringe cold email on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["shorter", "warmer", "more direct", "add a PS"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "To a design-tool founder: I built a Switchboard skin for their app and want to show a 2-minute demo.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Describe who you're emailing and the ask first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing the email…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write a short, specific, non-cringe cold email from the brief below.`,
        `THE BRIEF:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown with EXACTLY these bold headings, in order: **Subject** (under 6 words), **Body** (3–5 short lines: a genuine reason for reaching out, one concrete value line, one clear low-friction ask). Keep it human — no flattery, no buzzwords, no fake urgency. Never fabricate a shared connection or a false claim.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Email copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample brief (connect to write your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "who are you emailing, and what's the ask?…";
    ...
    const btn = el("button", "primary", "Write email ✉️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · subject · body · one clear ask"));
  ...
  bar.append(el("span", "kicker", "writing cold email"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the email"));
...
exposeToGod({
  name: "coldemail_run",
  description: "Write a short, non-cringe cold email (subject + body + one clear ask) from a target and an ask. Writes it live on the page and returns it.",
  inputSchema: { brief: "string — who you're emailing and the ask. Required." },
  execute: async ({ brief } = {}) => {
    const input = String(brief || "").trim();
    if (!input) throw new Error("nothing to write — pass { brief } with who and the ask");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Cold Email isn't connected to Switchboard yet");
    ...
    throw new Error("Cold Email stayed busy — try again");
  },
});
