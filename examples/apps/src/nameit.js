// NAME IT — describe a product, company, feature, or project and get six strong name options, each
// with a one-line rationale, one recommended, on the visitor's OWN Claude. The operator holds no
// key, pays for no inference, and never sees the user's data — Switchboard brokers everything.
//
...
// Option cards come from the shared kit (src/kit/ui.js): DRAFTED (the name Name It liked) stays
...
// God's hands: expose Name It's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "nameit",                                 // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Name It",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Name It — reads a description you paste and proposes strong name options on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = name things in the brand's own key
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// NAME IT — describe a product, company, feature, or project → stage 1 proposes 6 strong name
// options as option cards, each with a one-line rationale, one recommended (auto-selected). Clicking
// a card locks it (accent = a human chose) and copy grabs the name. Steering re-proposes all six.
// THE COLD OPEN: a lent brand context autostarts naming from the brand's own positioning. Text-only.

const STEER_CHIPS = ["shorter", "more playful", "more serious", "invented words"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "A calm, focused to-do app that hides everything except the one task you're doing right now.";
let running = false;

function autostart() {
  // THE COLD OPEN — when a brand context is lent, name from the brand's own positioning with ZERO
  // input. Fire only when useful, never over a saved run.
  if (state.run) return;
  if (brand) {
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Describe the thing you want to name first.", true); return; }
  // THREE ids, not one (doctrine 5). selectedId = the name currently shown as the working pick;
  // draftedId = Name It's own pick, tagged neutrally; chosenId = a card a HUMAN clicked, the only
  // card that ever wears the accent.
...
async function propose(steer) {
  ...
  running = true; r.error = null; r.status = "brainstorming names…"; render();
  try {
    const arr = await askJsonArray([
      `You are ${APP.name}, a naming expert. The user wants names for THIS:`,
      `"""${r.input.slice(0, 3000)}"""`,
      r.fromContext ? `(This is the user's OWN brand — name in a key that fits its existing positioning and voice.)` : "",
      r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
      "Propose 6 DISTINCT, strong name candidates — vary the approach (evocative, literal, invented/portmanteau, metaphor, short/punchy). Names should be memorable, pronounceable, and plausibly available. Avoid clichés and generic tech suffixes unless they genuinely fit.",
      'Return ONLY a JSON array — no prose, no fences. Each element: {"label":<the name>,"text":<one-line rationale for why this name works>,"recommended":<true for exactly one — the strongest overall>}',
    ]);
    if (!arr || !arr.length) throw new Error("no names came back — try again");
    r.options = arr.slice(0, 6).map((o) => ({ id: uid(), label: String(o.label || "Name").slice(0, 60), text: String(o.text || "").slice(0, 300), recommended: !!o.recommended }));
...
async function copyPick() {
  ...
  try { await navigator.clipboard.writeText(pick.label); toast("Name copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample description (connect to name your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · names in this key"));
    const row = el("div", "bindrow");
    const input = el("textarea");
    input.rows = 3;
    input.placeholder = "describe what you're naming — a product, company, feature, or project…";
    ...
    const btn = el("button", "primary", "Name it ✨"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · six options, each with a rationale"));
  ...
  bar.append(el("span", "kicker", "naming"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.options) {
    col.append(el("div", "kicker sect", r.chosenId ? "your name" : "six names — drafted, yours to pick"));
    col.append(optionCards({
      options: r.options,
      chosenId: r.chosenId,     // accent — a human clicked, nothing else
      draftedId: r.draftedId,   // neutral tag — Name It's pick, shown but not decided
      onChoose: (o) => choose(o.id, true),
      disabled: running,
      // Rule 4 — six names is a menu; this is the exit. Say the direction you'd want instead.
      escape: {
        label: "none of these — say the direction you'd want",
        placeholder: "the flavor you're after (e.g. 'one Latin root, two syllables')…",
        sendLabel: "rename",
...
// `nameit_run` runs the SAME start() a describe-and-go click runs — six names are proposed, live in
// the DOM — then returns them (with the recommended one) for God to speak.
exposeToGod({
  name: "nameit_run",
  description: "Propose six strong name options (each with a rationale) for a described product, company, feature, or project. Writes them live on the page and returns them.",
  inputSchema: { description: "string — a description of the thing to name. Required." },
  execute: async ({ description } = {}) => {
    const input = String(description || "").trim();
    if (!input) throw new Error("nothing to name — pass { description } with what to name");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Name It isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.options || r.error)) {
        ...
        return { names: (r.options || []).map((o) => ({ name: o.label, rationale: o.text })), recommended: rec ? rec.label : null };
      }
    }
    throw new Error("Name It stayed busy — try again");
  },
});
