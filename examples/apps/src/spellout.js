// SPELLOUT — drop a rough phrase or shorthand, get it expanded into a clear, well-formed sentence or
// short paragraph — same meaning, same voice — on the visitor's OWN Claude. The operator holds no
...
// God's hands: expose Spellout's one action as a page-tool so the native God webview (or any WebMCP
...
const APP = {
  id: "spellout",                               // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Spellout",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Spellout — expands a rough phrase into a clear, well-formed sentence on your own Claude",
    models: ["sonnet"],
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// SPELLOUT — drop a rough phrase, shorthand, or half-formed thought → one stage expands it into a
// clear, well-formed sentence or short paragraph that keeps the original meaning and voice, streamed
// as markdown. Steering re-writes it (shorter, more formal, one sentence…). Text-only — no tools.

const STEER_CHIPS = ["one sentence", "more formal", "warmer", "shorter"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "cant make the mtg tmrw, sick — resched for thurs?";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Type the rough phrase to spell out first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "spelling it out…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Expand the rough phrase below into clear, well-formed prose.`,
        `THE PHRASE:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Turn it into a clear, well-formed sentence or short paragraph. Keep the original meaning and the writer's voice — don't add facts, don't change the intent, don't pad it out. Fix grammar and shorthand into natural, readable English. Return ONLY the expanded text, no preamble, no options, no commentary.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample phrase (connect to spell out your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "drop a rough phrase, shorthand, or half-formed thought…";
    ...
    const btn = el("button", "primary", "Spell it out ✍️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · same meaning, same voice — just clearer"));
  ...
  bar.append(el("span", "kicker", "spelling out"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "spelled out"));
...
// `spellout_run` runs the SAME start() a type-and-go click runs — the expanded text streams live in
// the DOM — then returns it for God to speak or paste.
exposeToGod({
  name: "spellout_run",
  description: "Expand a rough phrase or shorthand into a clear, well-formed sentence or short paragraph, keeping the meaning and voice. Writes it live on the page and returns it.",
  inputSchema: { phrase: "string — the rough phrase to spell out. Required." },
  execute: async ({ phrase } = {}) => {
    const input = String(phrase || "").trim();
    if (!input) throw new Error("nothing to spell out — pass { phrase } with the rough phrase");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Spellout isn't connected to Switchboard yet");
    ...
    throw new Error("Spellout stayed busy — try again");
  },
});
