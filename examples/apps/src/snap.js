// SNAP ANSWER — ask a question, get a direct, concise answer on the visitor's OWN Claude. No chat
// thread, no ceremony — one question, one clean answer. The operator holds no key, pays for no
...
// God's hands: expose Snap Answer's one action as a page-tool so the native God webview (or any
...
const APP = {
  id: "snap",                                   // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Snap Answer",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Snap Answer — answers a question you type directly and concisely on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = answer with the brand's own facts
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// SNAP ANSWER — type a question → one stage answers it directly and concisely, streamed as markdown,
// on the user's OWN Claude. No chat thread, no follow-up ceremony — the answer leads, not a greeting.
// Steering nudges it (shorter, more detail, add an example…). THE COLD OPEN: a lent brand context
// autostarts an answer about the brand. Text-only — no tools, no image gen.

const STEER_CHIPS = ["shorter", "more detail", "add an example", "just the bottom line"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "What's the difference between margin and markup?";
let running = false;

function autostart() {
  // THE COLD OPEN — when a brand context is lent, answer a question about the brand with ZERO input.
  ...
  if (brand) {
    ...
    const seed = `In one line, what is ${brand.name} and who is it for? Context: ${bits.join(" — ") || brand.name}`;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Type a question first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "answering…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Answer the user's question directly and concisely — lead with the answer, no throat-clearing, no "great question", no restating the question.`,
        `THE QUESTION:\n"""${r.input.slice(0, 4000)}"""`,
        r.fromContext ? `(Answer using the brand's own facts where relevant.)` : "",
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Be accurate and specific. Use a short list only if it genuinely helps. If the question is ambiguous, answer the most likely reading and note the assumption in one clause. If you don't know or it can't be answered, say so plainly rather than guessing. Markdown.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1000,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Answer copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample question (connect to ask your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · answers use its facts"));
    const row = el("div", "bindrow");
    const input = el("textarea");
    input.rows = 3;
    input.placeholder = "ask anything — a clear, direct answer, no chat thread…";
    ...
    const btn = el("button", "primary", "Answer it ⚡"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · one question, one clean answer"));
  ...
  bar.append(el("span", "kicker", "answering"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the answer"));
...
// `snap_run` runs the SAME start() a type-and-go click runs — the answer streams live in the DOM —
// then returns the answer text for God to speak.
exposeToGod({
  name: "snap_run",
  description: "Answer a question directly and concisely on the user's own Claude. Writes the answer live on the page and returns it.",
  inputSchema: { question: "string — the question to answer. Required." },
  execute: async ({ question } = {}) => {
    const input = String(question || "").trim();
    if (!input) throw new Error("nothing to answer — pass { question } with the question");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Snap Answer isn't connected to Switchboard yet");
    ...
    throw new Error("Snap Answer stayed busy — try again");
  },
});
