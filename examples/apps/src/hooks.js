// HOOKS — turns a topic into scroll-stopping opening lines for posts, videos, and emails on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "hooks",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Hooks",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Hooks — turns a topic into scroll-stopping opening lines for posts, videos, and emails on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["punchier", "more contrarian", "add a stat", "shorter"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "A LinkedIn post about why most startup landing pages bury the value prop below the fold.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Tell it what the post is about first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing hooks…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write scroll-stopping opening lines (hooks) for the topic below.`,
        `THE TOPIC:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown as a numbered list of 7 hooks, each ONE line, no preamble. Vary the angles across the list — curiosity, contrarian, a concrete stat, a micro-story, a sharp question, a bold claim, a how-to. Mark the single strongest hook with **★ Recommended** and add one short line on why it wins. Never invent facts the topic doesn't support.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Hooks copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample topic (connect to write your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "what's the post/video/email about?…";
    ...
    const btn = el("button", "primary", "Write hooks ✍️"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · 7 hooks · one recommended"));
  ...
  bar.append(el("span", "kicker", "writing hooks"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the hooks"));
...
exposeToGod({
  name: "hooks_run",
  description: "Write 7 scroll-stopping opening lines (hooks) for a topic, one marked recommended. Writes them live on the page and returns them.",
  inputSchema: { topic: "string — what the post/video/email is about. Required." },
  execute: async ({ topic } = {}) => {
    const input = String(topic || "").trim();
    if (!input) throw new Error("nothing to hook — pass { topic } describing the post");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Hooks isn't connected to Switchboard yet");
    ...
    throw new Error("Hooks stayed busy — try again");
  },
});
