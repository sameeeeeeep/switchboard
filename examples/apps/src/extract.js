// EXTRACT — paste anything, get the key fields and entities pulled into a clean labeled list, on the
...
// God's hands: expose Extract's one action as a page-tool so the native God webview (or any WebMCP
// host) can DRIVE it — reusing the same start() a click runs, so the user watches it happen.
import { exposeToGod } from "./kit/webmcp.js";

// ==== CONFIG — every new wrapp edits this block =============================================
const APP = {
  id: "extract",                                // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Extract",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Extract — pulls the key fields and entities out of text you paste, on your own Claude",
    models: ["sonnet"],
    tools: [],                                  // text-only; no tools, no image gen
  },
  usesContext: "single",                        // a lent context = extract fields from the brand's own data
  placeholder: "paste anything — an email, an invoice, a bio, a listing, a contract…",
  extraField: null,
};
...
// ==== APP LOGIC ═════════════════════════════════════════════════════════════════════════════
// EXTRACT — paste anything → stream the key fields, entities, and facts as a clean set of labeled
// lines (**Label:** value). Steering re-runs the extraction. THE COLD OPEN: a lent brand context
// autostarts an extraction from the brand's own data with zero input. Text-only — no tools.

const STEER_CHIPS = ["more fields", "fewer fields", "add values", "just names"];
const SAMPLE = "Invoice #4821 — Acme Corp, 500 Market St, San Francisco. Bill to: Jane Doe, jane@northwind.io. Due 2026-08-15. 3× Widget Pro @ $120, 1× Onboarding @ $400. Subtotal $760, tax $68.40, total $828.40. Net 30.";
let running = false;
...
async function start(input, extra, fromContext) {
  ...
  if (!input) { toast("Paste something to extract from first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.output = ""; r.status = "pulling the fields…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Pull the key fields, entities, and facts out of the text below into a clean, labeled list.`,
        `TEXT:\n"""${r.input.slice(0, 8000)}"""`,
        r.fromContext ? `(This is the user's OWN brand data — extract its key attributes.)` : "",
        r.steer ? `Adjust per this instruction: "${r.steer}".` : "",
        "Output labeled lines in the form **Label:** value — one per line, grouped sensibly. Include ONLY fields actually present in the text; never invent values. No preamble, no prose. Markdown.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1200,
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample (connect to extract from your own)"));
  ...
  if (!r) {
    ...
    if (brand) startBox.append(el("div", "ctx", "loaded your lent context — " + brand.name + " · about to extract its key fields"));
    ...
    const btn = el("button", "primary", "Extract fields"); btn.onclick = go;
    startBox.append(btn);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter to run · clean labeled fields, nothing invented"));
  ...
  if (r.output) {
    col.append(el("div", "kicker sect", "the fields"));
...
exposeToGod({
  name: "extract_run",
  description: "Pull the key fields and entities out of a bit of text into a clean labeled list. Writes it live on the page and returns it.",
  inputSchema: { text: "string — the text to extract fields from. Required." },
  execute: async ({ text } = {}) => {
    const input = String(text || "").trim();
    if (!input) throw new Error("nothing to extract — pass { text } with the text to extract from");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Extract isn't connected to Switchboard yet");
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.input === input && (r.output || r.error)) {
        ...
        return { fields: r.output };
      }
    }
    throw new Error("Extract stayed busy — try again");
  },
});
