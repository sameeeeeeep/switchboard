// CAPTION — turns a photo idea or note into social captions with hashtags on your own Claude. On the visitor's OWN Claude. The operator holds no key, pays for no inference, and
...
const APP = {
  id: "caption",                                  // = build.mjs entry name = ./dist/<id>.js in the html
  name: "Caption",
  installUrl: "https://thelastprompt.ai/switchboard/",
  scope: {
    reason: "Caption — turns a photo idea or note into social captions with hashtags on your own Claude",
    models: ["sonnet"],
...
const STEER_CHIPS = ["funnier", "more aesthetic", "fewer hashtags", "add a CTA"];
// Pre-connect ONLY — a visibly-labeled sample so the empty state isn't dead. Gone the moment Claude connects.
const SAMPLE = "A behind-the-scenes shot of our team shipping the v1 launch at 2am, empty coffee cups everywhere.";
let running = false;
...
async function start(input, fromContext) {
  ...
  if (!input) { toast("Describe the post or photo first.", true); return; }
...
async function run(steer) {
  ...
  running = true; r.error = null; r.out = ""; r.status = "writing captions…"; render();
  try {
    const text = await streamText({
      prompt: [
        `You are ${APP.name}. Write social captions for the post described below.`,
        `THE POST:\n"""${r.input.slice(0, 12000)}"""`,
        r.steers.length ? `Steering (apply the latest): ${r.steers.map((s) => `"${s}"`).join(" → ")}` : "",
        "Reply in markdown: a numbered list of 5 caption options that vary in length and vibe (punchy, warm, witty, minimal, story). Mark the single best with **★ Recommended**. Then a final **Hashtags** line with 8–12 relevant tags. Keep it platform-agnostic unless the post names one.",
      ].filter(Boolean).join("\n\n"),
      maxTokens: 1600,
...
async function copyOut() {
  ...
  try { await navigator.clipboard.writeText(r.out); toast("Captions copied ✓"); }
...
function render() {
  ...
  if (!relay) {
    ...
    s.append(el("div", "kicker", "sample post (connect to caption your own)"));
  ...
  if (!r) {
    ...
    input.rows = 3;
    input.placeholder = "describe the post or photo…";
    ...
    const btn = el("button", "primary", "Caption it 📸"); btn.onclick = go;
    row.append(input, btn);
    startBox.append(row);
    startBox.append(el("div", "hint", "⌘/Ctrl + Enter · 5 captions · hashtags"));
  ...
  bar.append(el("span", "kicker", "captioning"), el("span", "run-input", r.input), el("span", "grow"));
  ...
  if (r.out) {
    col.append(el("div", "kicker sect", "the captions"));
...
exposeToGod({
  name: "caption_run",
  description: "Write 5 social captions plus a hashtag set for a post, one marked recommended. Writes them live on the page and returns them.",
  inputSchema: { about: "string — what the post or photo is. Required." },
  execute: async ({ about } = {}) => {
    const input = String(about || "").trim();
    if (!input) throw new Error("nothing to caption — pass { about } describing the post");
    ...
    if (!await waitFor(() => !!relay, 6000)) throw new Error("Caption isn't connected to Switchboard yet");
    ...
    throw new Error("Caption stayed busy — try again");
  },
});
