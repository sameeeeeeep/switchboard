...
_Status: **design**, written 2026-07-26. The audit findings in §7 are verified against the tree.
**Steps 1–2 of §9 have shipped** (the 404, the `Relay`→`Switchboard` copy drift, and `installedHere`
...
| 5 connect | **page chip** (per-origin, on the page) | panel lists the site as not-connected |
...
- The remediation string moves into the shared copy table (§6) so all three surfaces say it identically.
...
`inherited: "unknown"` is the honest default until the enumeration question is solved (§8). Every
...
## 7. Verified defects, ranked

Everything below is confirmed against the tree at `04f3469`.

### 7.1 The download button 404s — ✅ FIXED 2026-07-26
...
1. ✅ **Fix the 404** (§7.1) and the `Relay` → `Switchboard` copy drift.
...
4. **One copy table** (§6), retiring the three stale per-app mappers.
