...
mkdir -p build
swiftc -O -o build/Relay main.swift RelayMenuBar.swift CursorGuide.swift TeamSection.swift TeamNotchFlow.swift TeamCursorsOverlay.swift IgnitionOverlay.swift DictationScratch.swift WhiteboardPanel.swift NotchTray.swift NotchLauncherView.swift LauncherRouting.swift GodWidgetKit.swift GodWebWindow.swift StoreFrontView.swift HtmlCapability.swift SkillRunner.swift AmbientSensor.swift AmbientCanvas.swift OSShellView.swift OSSurfaceWorkspace.swift OSSurfaceAutomate.swift OSSurfaceKnowledge.swift OSSurfaceDo.swift -framework AppKit -framework SwiftUI -framework WebKit -framework ApplicationServices -framework CoreServices
...
# MacCat pester sprites — the animated pixel cat + behaviour that chases the cursor on a /hijack
# (TeamCursorsOverlay loads Resources/sprites/cat/<frame>.png). Copied RECURSIVELY (the cat/ subdir of
# per-action frames). Without this a deployed build ships no cats → the fallback SF cat glyph.
if [ -d assets/sprites ]; then
  mkdir -p "$APP/Contents/Resources/sprites"
  cp -R assets/sprites/. "$APP/Contents/Resources/sprites/" 2>/dev/null || true
